...
function removeTrailingWhitespaces(value) {
  // Use regex to remove only trailing whitespaces
...
function repeatString(/* str, times */) {
  // if (str !== '' || !str < 0) {
  //   const result = str.toString().repeat(times);
  // }
...
function formatTime(minutes, seconds) {
  ...
  const result = `${formatedMinutes}:${formatedSeconds}`;
  return result;
}
...
function orderAlphabetically(str) {
  const result = Array.from(str).sort().join('');
  return result;
}
...
function containsSubstring(str, substring) {
  const result = String(str).includes(substring);
  return result;
}
...
function countVowels(str) {
  const vowels = str.match(/[aeiouyEAIOUY]/g);
  if (vowels) {
...
function findLongestWord(sentence) {
  const result = sentence.split(' ').reduce((b, a) => {
...
function extractNameFromTemplate(value) {
  ...
  const result = value.slice(index, value.length - 1);
  return result;
}
...
function getCardId(/* value */) {
  ...
  //   'K',
  // ];
  // const cardValue = cards.slice(0, 1);
  // return cardValue;
  throw new Error('Not implemented');
}
